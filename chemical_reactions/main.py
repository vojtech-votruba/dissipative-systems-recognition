...
fig5, axes5 = plt.subplots(2, int(np.ceil(DIMENSION / 2)))
axes5 = axes5.flatten()
...
for d in range(DIMENSION):
    ...
    axes5[d].plot(x_independent.cpu(), dS_predicted[...,d].cpu().detach().numpy(), label=r"learned")

    graph.set_xlabel(f"$c_{d+1}$")
    graph.set_ylabel(r"$S$")

    axes5[d].set_xlabel(f"$c_{d+1}$")
    axes5[d].set_ylabel(f"$\partial S/ \partial c_{d+1}$")
...
fig5.savefig("results/dentr.pdf")
plt.show()
